...
// ########## METHOD PUBLIC
    ...
    *
    * @param mysqli_result $rRslt Takes the result of the query
   ...
   public function query (&$rRslt, $sQuery)
   {
      ...
      try
      {
         ...
         {
            $iErr = 1; // query failed
            lib\ErrLog::getInstance()->push("{" . __METHOD__ . "} [err#: " . mysqli_errno($this->_rLink) . ", " . mysqli_error($this->_rLink) . "] For query[" . $sQuery . "]");
         }
      }
      catch (\mysql_xdevapi\Exception $e)
      {
         $iErr = 1; // query failed
         lib\ErrLog::getInstance()->push("{" . __METHOD__ . "} [exception #: " . $e->getCode() . ", " . $e->getMessage() . "] For query[" . $sQuery . "]");
      }
...
// TRANSACTIONS
    ...
    * ist $aArray false.
    * </p>
    * @param array $aArray Reference in which load result rows. Is bool FALSE after last row.
    * @param mysqli_result $rRslt Recordset from fetch data.
    * @param int $iType Defines the type of the returned array:<br>
    ...
    * <p>
    * Free up memory resources used in the query function.<br>
    * This feature is useful on servers with very little memory to free up its resources.
    * Once the script is run, the memory will have been released and available again.
    * Note that after running this function, the {@see $rRslt} parameter referring to the recordset is no longer valid.
    * </p>
    * @param resource $rRslt Recordset object returned from a query
    * @return int fixed 0 value
    */
